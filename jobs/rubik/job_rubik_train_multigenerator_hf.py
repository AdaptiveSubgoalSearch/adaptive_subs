...
class TrainHfForRubikMultigenerator(HfTrainingPipelineRubik):
    def __init__(
        self,
        n_proofs=10000,
        n_val_proofs=2,
        ...
        **kwargs
        ...
        super().__init__(
            tokenizer=hf_rubik_subgoal.RubikGoalTokenizer(
                model_max_length=hf_rubik_subgoal.SEQUENCE_LENGTH + 1,  # additional distance token
                padding_side='right',
            ...
            model_config_overrides=hf_utils.ModelConfigOverrides(
                ...
                max_position_embeddings=hf_rubik_subgoal.SEQUENCE_LENGTH + 1,
                min_length=hf_rubik_subgoal.SEQUENCE_LENGTH + 1,
                vocab_size=len(hf_rubik_subgoal.VOCABULARY)),
            **kwargs
        )
        self.n_proofs = n_proofs
        self.n_val_proofs = n_val_proofs
        self.max_seq_length = hf_rubik_subgoal.SEQUENCE_LENGTH + 1
    ...
    def _log_validity_metrics(self, dataset, sequences, tokenizer, done_epochs, prefix, model=None):
        valid_rate, simple_valid_rate, fixed_valid_rate = gen_rubik_data.check_valid(sequences[:20])
